...
static inline void clock_generator(uint gpio_pin, float mhz) {
    gpio_set_function(gpio_pin, GPIO_FUNC_PWM);
    uint slice_num = pwm_gpio_to_slice_num(gpio_pin);

    pwm_config c = pwm_get_default_config();
    pwm_config_set_clkdiv(&c, clock_get_hz(clk_sys) / (2.0f * mhz));
    pwm_config_set_wrap(&c, 3);//MAX PWM value
    pwm_init(slice_num, &c, true);
    pwm_set_gpio_level(gpio_pin, 2);
...
int __time_critical_func(main)() {
    overclock();
    clock_generator(CLCK_PIN, 3'579'545.0f);
